...
export const CartFooter = ({ onPay, className }: CartFooterProps) => {
  ...
  return (
    <div
      className={cn(
        'grid grid-cols-2 gap-x-3 gap-y-6 md:flex md:items-center md:gap-14',
        className,
      )}
    >
      {total > 0 && (
      ...
      <Button
        ...
        onClick={() => router.push('/')}
        className={cn(
          'w-full md:order-1 md:w-auto md:min-w-[260px]',
      ...
      {total > 0 && (
        <Button
          variant="primary"
          onClick={onPay}
          className="col-span-1 w-full md:order-3 md:w-auto md:min-w-[260px]"
        >
          Pay
        </Button>
      )}
